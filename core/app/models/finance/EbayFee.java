...
public class EbayFee extends Model {
    ...
    public static boolean parseFinancesApiResult(ListFinancialEventsResponse response, EbayOrder order) {
        ...
        if(financialEvents.getShipmentEventList().size() > 0) {
            ...
            events.forEach(event -> event.getShipmentItemList().forEach(item -> {
                        item.getItemFeeList().forEach(
                                component -> EbayFee.saveItemFeeForApi(component, event, item, "Order", order));
                    }
            ));
        }
        return true;
    ...
    private static void saveItemFeeForApi(FeeComponent component, ShipmentEvent event, ShipmentItem item, String type,
        ...
        if(component.getFeeAmount().getCurrencyAmount().floatValue() != 0) {
            ...
            EbayFee.commonField(fee, event, item, type, order);
        }
    }

    private static void commonField(EbayFee fee, ShipmentEvent event, ShipmentItem item, String type, EbayOrder order) {
