package com.amazonservices.mws.FulfillmentInventory._2010_10_01.model;

import javax.xml.bind.annotation.*;
import javax.xml.datatype.XMLGregorianCalendar;


/**
 * <p>Java class for anonymous complex type.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * 
 * <pre>
 * &lt;complexType>
 *   &lt;complexContent>
 *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *       &lt;sequence>
 *         &lt;element name="SellerId" type="{http://www.w3.org/2001/XMLSchema}string"/>
 *         &lt;element name="Marketplace" type="{http://www.w3.org/2001/XMLSchema}string"/>
 *         &lt;element name="SellerSkus" type="{http://mws.amazonaws.com/FulfillmentInventory/2010-10-01/}SellerSkuList" minOccurs="0"/>
 *         &lt;element name="QueryStartDateTime" type="{http://www.w3.org/2001/XMLSchema}dateTime" minOccurs="0"/>
 *         &lt;element name="ResponseGroup" type="{http://www.w3.org/2001/XMLSchema}string" minOccurs="0"/>
 *       &lt;/sequence>
 *     &lt;/restriction>
 *   &lt;/complexContent>
 * &lt;/complexType>
 * </pre>
 * Generated by AWS Code Generator
 * <p/>
 * Fri Oct 22 09:47:28 UTC 2010
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "", propOrder = {
    "sellerId",
    "marketplace",
    "sellerSkus",
    "queryStartDateTime",
    "responseGroup"
})
@XmlRootElement(name = "ListInventorySupplyRequest")
public class ListInventorySupplyRequest {

    @XmlElement(name = "SellerId", required = true)
    protected String sellerId;
    @XmlElement(name = "Marketplace", required = true)
    protected String marketplace;
    @XmlElement(name = "SellerSkus")
    protected SellerSkuList sellerSkus;
    @XmlElement(name = "QueryStartDateTime")
    @XmlSchemaType(name = "dateTime")
    protected XMLGregorianCalendar queryStartDateTime;
    @XmlElement(name = "ResponseGroup")
    protected String responseGroup;

    /**
     * Default constructor
     * 
     */
    public ListInventorySupplyRequest() {
        super();
    }

    /**
     * Value constructor
     * 
     */
    public ListInventorySupplyRequest(final String sellerId, final String marketplace, final SellerSkuList sellerSkus, final XMLGregorianCalendar queryStartDateTime, final String responseGroup) {
        this.sellerId = sellerId;
        this.marketplace = marketplace;
        this.sellerSkus = sellerSkus;
        this.queryStartDateTime = queryStartDateTime;
        this.responseGroup = responseGroup;
    }

    /**
     * Gets the value of the sellerId property.
     * 
     * @return
     *     possible object is
     *     {@link String }
     *     
     */
    public String getSellerId() {
        return sellerId;
    }

    /**
     * Sets the value of the sellerId property.
     * 
     * @param value
     *     allowed object is
     *     {@link String }
     *     
     */
    public void setSellerId(String value) {
        this.sellerId = value;
    }

    public boolean isSetSellerId() {
        return (this.sellerId!= null);
    }

    /**
     * Gets the value of the marketplace property.
     * 
     * @return
     *     possible object is
     *     {@link String }
     *     
     */
    public String getMarketplace() {
        return marketplace;
    }

    /**
     * Sets the value of the marketplace property.
     * 
     * @param value
     *     allowed object is
     *     {@link String }
     *     
     */
    public void setMarketplace(String value) {
        this.marketplace = value;
    }

    public boolean isSetMarketplace() {
        return (this.marketplace!= null);
    }

    /**
     * Gets the value of the sellerSkus property.
     * 
     * @return
     *     possible object is
     *     {@link SellerSkuList }
     *     
     */
    public SellerSkuList getSellerSkus() {
        return sellerSkus;
    }

    /**
     * Sets the value of the sellerSkus property.
     * 
     * @param value
     *     allowed object is
     *     {@link SellerSkuList }
     *     
     */
    public void setSellerSkus(SellerSkuList value) {
        this.sellerSkus = value;
    }

    public boolean isSetSellerSkus() {
        return (this.sellerSkus!= null);
    }

    /**
     * Gets the value of the queryStartDateTime property.
     * 
     * @return
     *     possible object is
     *     {@link javax.xml.datatype.XMLGregorianCalendar }
     *     
     */
    public XMLGregorianCalendar getQueryStartDateTime() {
        return queryStartDateTime;
    }

    /**
     * Sets the value of the queryStartDateTime property.
     * 
     * @param value
     *     allowed object is
     *     {@link javax.xml.datatype.XMLGregorianCalendar }
     *     
     */
    public void setQueryStartDateTime(XMLGregorianCalendar value) {
        this.queryStartDateTime = value;
    }

    public boolean isSetQueryStartDateTime() {
        return (this.queryStartDateTime!= null);
    }

    /**
     * Gets the value of the responseGroup property.
     * 
     * @return
     *     possible object is
     *     {@link String }
     *     
     */
    public String getResponseGroup() {
        return responseGroup;
    }

    /**
     * Sets the value of the responseGroup property.
     * 
     * @param value
     *     allowed object is
     *     {@link String }
     *     
     */
    public void setResponseGroup(String value) {
        this.responseGroup = value;
    }

    public boolean isSetResponseGroup() {
        return (this.responseGroup!= null);
    }

    /**
     * Sets the value of the SellerId property.
     * 
     * @param value
     * @return
     *     this instance
     */
    public ListInventorySupplyRequest withSellerId(String value) {
        setSellerId(value);
        return this;
    }

    /**
     * Sets the value of the Marketplace property.
     * 
     * @param value
     * @return
     *     this instance
     */
    public ListInventorySupplyRequest withMarketplace(String value) {
        setMarketplace(value);
        return this;
    }

    /**
     * Sets the value of the SellerSkus property.
     * 
     * @param value
     * @return
     *     this instance
     */
    public ListInventorySupplyRequest withSellerSkus(SellerSkuList value) {
        setSellerSkus(value);
        return this;
    }

    /**
     * Sets the value of the QueryStartDateTime property.
     * 
     * @param value
     * @return
     *     this instance
     */
    public ListInventorySupplyRequest withQueryStartDateTime(XMLGregorianCalendar value) {
        setQueryStartDateTime(value);
        return this;
    }

    /**
     * Sets the value of the ResponseGroup property.
     * 
     * @param value
     * @return
     *     this instance
     */
    public ListInventorySupplyRequest withResponseGroup(String value) {
        setResponseGroup(value);
        return this;
    }
    


    /**
     *
     * JSON fragment representation of this object
     *
     * @return JSON fragment for this object. Name for outer
     * object expected to be set by calling method. This fragment
     * returns inner properties representation only
     *
     */
    protected String toJSONFragment() {
        StringBuffer json = new StringBuffer();
        boolean first = true;
        if (isSetSellerId()) {
            if (!first) json.append(", ");
            json.append(quoteJSON("SellerId"));
            json.append(" : ");
            json.append(quoteJSON(getSellerId()));
            first = false;
        }
        if (isSetMarketplace()) {
            if (!first) json.append(", ");
            json.append(quoteJSON("Marketplace"));
            json.append(" : ");
            json.append(quoteJSON(getMarketplace()));
            first = false;
        }
        if (isSetSellerSkus()) {
            if (!first) json.append(", ");
            json.append("\"SellerSkus\" : {");
            SellerSkuList  sellerSkus = getSellerSkus();


            json.append(sellerSkus.toJSONFragment());
            json.append("}");
            first = false;
        }
        if (isSetQueryStartDateTime()) {
            if (!first) json.append(", ");
            json.append(quoteJSON("QueryStartDateTime"));
            json.append(" : ");
            json.append(quoteJSON(getQueryStartDateTime() + ""));
            first = false;
        }
        if (isSetResponseGroup()) {
            if (!first) json.append(", ");
            json.append(quoteJSON("ResponseGroup"));
            json.append(" : ");
            json.append(quoteJSON(getResponseGroup()));
            first = false;
        }
        return json.toString();
    }

    /**
     *
     * Quote JSON string
     */
    private String quoteJSON(String string) {
        StringBuffer sb = new StringBuffer();
        sb.append("\"");
        int length = string.length();
        for (int i = 0; i < length; ++i) {
            char c = string.charAt(i);
            switch (c) {
            case '"':
                sb.append("\\\"");
                break;
            case '\\':
                sb.append("\\\\");
                break;
            case '/':
                sb.append("\\/");
                break;
            case '\b':
                sb.append("\\b");
                break;
            case '\f':
                sb.append("\\f");
                break;
            case '\n':
                sb.append("\\n");
                break;
            case '\r':
                sb.append("\\r");
                break;
            case '\t':
                sb.append("\\t");
                break;
            default:
                if (c <  ' ') {
                    sb.append("\\u" + String.format("%03x", Integer.valueOf(c)));
                } else {
                sb.append(c);
            }
        }
        }
        sb.append("\"");
        return sb.toString();
    }


}
