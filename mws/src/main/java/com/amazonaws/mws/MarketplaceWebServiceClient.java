...
public  class MarketplaceWebServiceClient implements MarketplaceWebService {
    ...
    private HttpClient configureHttpClient(
        ...
        defaultHeaders.add(new Header("X-Amazon-User-Agent", config.getUserAgent()));
        defaultHeaders.add(new Header("Accept-Encoding", "gzip"));
    ...
    private <T, U> T invoke(Class<T> clazz, Map<String, String> parameters, U request ) throws MarketplaceWebServiceException {
        ...
        try {
            if(!config.isSetServiceURL()){
                throw new MarketplaceWebServiceException("Missing serviceUrl configuration value. You may obtain a list of valid MWS URLs by consulting the MWS Developer's Guide, or reviewing the sample code published along side this library.", -1, "InvalidServiceUrl", "Sender", null,  null) {
                };
            }
            ...
            if( request instanceof SubmitFeedRequest ) {
                ...
                // 自己在下载 Report 的地方添加了 Gzip, 以减少传输
            ...
            do {
                ...
                try {
                    ...
                    if( !isStreamingResponse ) {
                        // SubmitFeed
                        responseBodyString = getResponsBodyAsString(isGzip(method) ?
                                new GZIPInputStream(method.getResponseBodyAsStream()) : method.getResponseBodyAsStream());
                    ...
                    if (status == HttpStatus.SC_OK && responseBodyString != null) {
                        ...
                        copyResponseToOutputStream(isGzip(method) ? new GZIPInputStream(method.getResponseBodyAsStream()) : method.getResponseBodyAsStream(), os);
                        ...
                        if( isStreamingResponse ) {
                            ...
                            responseBodyString = getResponsBodyAsString(isGzip(method) ? new GZIPInputStream(method.getResponseBodyAsStream()) : method.getResponseBodyAsStream());
                        }
     ...
     * @return
     */
    private boolean isGzip(PostMethod method) {
        boolean gzip = false;
        Header gzipHeader = method.getResponseHeader("Content-Encoding");
        if(gzipHeader != null) { // 自己添加的 Gzip
            gzip = gzipHeader.getValue().toLowerCase().contains("gzip");
        }
        return gzip;
    }
    ...
    private MarketplaceWebServiceException processErrors(String responseString, int status)  {
        ...
        if (responseString != null && responseString.startsWith("<")) {
            ...
            if (matcher.matches()) {
                ...
                ex =  new MarketplaceWebServiceException("Internal Error", status);
